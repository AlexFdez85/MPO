...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['eliminar_venta'])) {
    $ordenId = (int)($_POST['orden_id_delete'] ?? ($_GET['id'] ?? 0));
    if (!$ordenId) {
        header('Location: ordenes_venta.php?error='.urlencode('Falta id de orden'));
        exit;
    }
    ...
    try {
        ...
        // 1) Borrar surtidos asociados
        $pdo->prepare("
            DELETE sv FROM surtidos_venta sv
            JOIN lineas_venta lv ON lv.id = sv.linea_venta_id
            WHERE lv.orden_venta_id = ?
        ")->execute([$ordenId]);

        // 2) Borrar entregas
        $pdo->prepare("DELETE FROM entregas_venta WHERE orden_venta_id = ?")
            ->execute([$ordenId]);

        // 3) Borrar reservas (activas o consumidas)
        $pdo->prepare("
            DELETE rv FROM reservas_venta rv
            JOIN lineas_venta lv ON lv.id = rv.linea_venta_id
            WHERE lv.orden_venta_id = ?
        ")->execute([$ordenId]);

        // 4) Borrar líneas
        $pdo->prepare("DELETE FROM lineas_venta WHERE orden_venta_id = ?")
            ->execute([$ordenId]);

        // 5) Borrar cabecera
        $pdo->prepare("DELETE FROM ordenes_venta WHERE id = ?")
            ->execute([$ordenId]);
        ...
        $pdo->rollBack();
        header('Location: ordenes_venta.php?error='.urlencode('No se pudo eliminar: '.$e->getMessage()));
    }
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['crear_venta_ajax'])) {
    ...
    try {
        ...
        // Helper: disponible actual (físico - apartado en OVs activas)
        $getDisponible = function(PDO $pdo, int $prodId, int $presId): array {
            $sql = "
              SELECT
                COALESCE((SELECT SUM(pt.cantidad)
                          FROM productos_terminados pt
                          WHERE pt.producto_id = ? AND pt.presentacion_id = ?),0) AS stock,
                COALESCE((
                  SELECT SUM(GREATEST(lv.cantidad - COALESCE(sv.cant_surtida,0),0))
                  FROM lineas_venta lv
                  JOIN ordenes_venta ov
                    ON ov.id = lv.orden_venta_id
                   AND ov.estado NOT IN ('entregado','cancelado')
                  LEFT JOIN (
                    SELECT linea_venta_id, SUM(cantidad) AS cant_surtida
                    FROM surtidos_venta GROUP BY 1
                  ) sv ON sv.linea_venta_id = lv.id
                  WHERE lv.producto_id = ? AND lv.presentacion_id = ?
                ),0) AS apartado
            ";
            $st = $pdo->prepare($sql);
            $st->execute([$prodId,$presId,$prodId,$presId]);
            $r = $st->fetch(PDO::FETCH_ASSOC) ?: ['stock'=>0,'apartado'=>0];
            $disp = max(0, (float)$r['stock'] - (float)$r['apartado']);
            return ['stock'=>(float)$r['stock'], 'apartado'=>(float)$r['apartado'], 'disponible'=>$disp];
        };
        ...
        $stmtReq   = $pdo->prepare("INSERT INTO requerimientos_produccion (orden_venta_id, linea_venta_id, producto_id, presentacion_id, cantidad_requerida, estado, fecha_objetivo, creado_por) VALUES (?, ?, ?, ?, ?, 'pendiente', ?, ?)");
        foreach ($prodIds as $i => $pid) {
            $pid = intval($pid);
            $pr = intval($presIds[$i] ?? 0);
            $ct = floatval($cants[$i] ?? 0);
            if ($pid > 0 && $pr > 0 && $ct > 0) {
                // Disponible hoy y faltante
                $dispInfo = $getDisponible($pdo, $pid, $pr);
                $reservar = min($ct, $dispInfo['disponible']);
                $faltante = max(0,  $ct - $reservar);

                // 1) Insertar línea
                $stmtLinea->execute([$ventaId, $pid, $pr, $ct]);
                $lineaId = (int)$pdo->lastInsertId();

                // 2) Registrar requerimiento de producción por la diferencia
                if ($faltante > 0) {
                    $fechaObjetivo = !empty($fechaEntrega) ? $fechaEntrega : date('Y-m-d');
                    $stmtReq->execute([$ventaId, $lineaId, $pid, $pr, $faltante, $fechaObjetivo, $usuario_id]);
                }
            }
        }
...
// 6) Actualizar estados según avance y entregas (orden correcto y una sola vez)
// 6.a) entregado si hay registro en entregas_venta
$pdo->exec("
   UPDATE ordenes_venta ov
   JOIN   entregas_venta e ON e.orden_venta_id = ov.id
   SET    ov.estado = 'entregado'
   WHERE  ov.estado <> 'entregado'
");
// 6.b) Listo_entrega si ya surtió todo y aún no está entregado
$pdo->exec("
   UPDATE ordenes_venta ov
   LEFT JOIN (
     SELECT orden_venta_id, SUM(cantidad) AS total_pedida
     FROM lineas_venta GROUP BY orden_venta_id
   ) lp ON lp.orden_venta_id = ov.id
   LEFT JOIN (
     SELECT orden_venta_id, SUM(cantidad) AS total_reservado
     FROM reservas_venta
     WHERE estado = 'activa'
     GROUP BY orden_venta_id
   ) ls ON ls.orden_venta_id = ov.id
   SET ov.estado = 'Listo_entrega'
    WHERE COALESCE(ls.total_reservado,0) >= COALESCE(lp.total_pedida,0)
     AND ov.estado <> 'entregado'
");
// 6.c) en_proceso si tiene algo surtido pero no completo
$pdo->exec("
   UPDATE ordenes_venta ov
   LEFT JOIN (SELECT orden_venta_id, SUM(cantidad) AS total_pedida
              FROM lineas_venta GROUP BY orden_venta_id) lp ON lp.orden_venta_id = ov.id
   LEFT JOIN (SELECT orden_venta_id, SUM(cantidad) AS total_reservado
              FROM reservas_venta WHERE estado='activa'
              GROUP BY orden_venta_id) ls ON ls.orden_venta_id = ov.id
SET ov.estado = 'proceso_surtido'
  WHERE COALESCE(ls.total_reservado,0) > 0
    AND COALESCE(ls.total_reservado,0) < COALESCE(lp.total_pedida,0)
     AND ov.estado NOT IN ('entregado','Listo_entrega')
");
// 6.d) pendiente si no hay surtidos y no está entregado
$pdo->exec("
   UPDATE ordenes_venta ov
   LEFT JOIN (SELECT orden_venta_id, SUM(cantidad) AS total_reservado
              FROM reservas_venta WHERE estado='activa'
              GROUP BY orden_venta_id) ls ON ls.orden_venta_id = ov.id
   SET ov.estado = 'pendiente'
   WHERE COALESCE(ls.total_reservado,0) = 0
     AND ov.estado <> 'entregado'
");
...
// 7) Listar pedidos existentes + cálculo de avance
$ordenes = $pdo->query("
  SELECT
    ov.id,
    c.nombre AS cliente,
    ...
    u.nombre AS creador,
    ov.incluye_paquete,
    COALESCE(lv.total_pedida,0)  AS total_pedida,
    COALESCE(rv.total_reservado,0) AS total_surtida,
    CASE
      WHEN COALESCE(lv.total_pedida,0) = 0 THEN 0
      ELSE ROUND(100 * COALESCE(rv.total_reservado,0) / lv.total_pedida)
    END AS avance
  ...
  LEFT JOIN (
    SELECT orden_venta_id, SUM(cantidad) AS total_pedida
    FROM lineas_venta GROUP BY orden_venta_id
  ) lv ON lv.orden_venta_id = ov.id
  LEFT JOIN (
    SELECT orden_venta_id, SUM(cantidad) AS total_reservado
    FROM reservas_venta
    WHERE estado='activa'
    GROUP BY orden_venta_id
  ) rv ON rv.orden_venta_id = ov.id
  ORDER BY ov.id DESC
")->fetchAll(PDO::FETCH_ASSOC);
...
<!-- Botón: Mostrar/Ocultar Registro de Pedido (oculto para produccion) -->
...
<div class="container mt-4">
    ...
    <div class="collapse" id="registroPedido">
        ...
        <div class="card mb-4 p-3">
            ...
            <form method="post" id="pedido-form" class="row g-3 mt-3" action="ordenes_venta.php">
                <!-- Flag para que el backend responda JSON en la rama AJAX -->
                <input type="hidden" name="crear_venta_ajax" value="1">
                ...
                <div class="col-md-2 text-end">
                   <!-- No submit directo: abre modal de confirmación -->
                    <button type="button" id="open-confirm-modal-btn" class="btn btn-success w-100">Crear Pedido</button>
                </div>
                ...
                <div class="col-12">
                    ...
                    <table class="table table-bordered mb-0">
                        ...
                        <!-- Fila plantilla: OCULTA y DESHABILITADA para que no entre a validación -->
                        <tr id="template-row" style="display:none;">
                            <td>
                                <select name="producto_id[]" class="form-select form-select-sm producto-select" required
                                        onchange="fetchPresentations(this)" disabled>
                            ...
                            <td>
                                <select name="presentacion_id[]" class="form-select form-select-sm presentation-select" required disabled>
                                    <option value="">-- Elige presentación --</option>
                                </select>
                            </td>
                            <td>
                                <input name="cantidad[]" type="number" step="1" min="1" class="form-control form-control-sm" required disabled>
                            </td>
...
<?php endif; ?>
    ...
    <div class="tab-content" id="tabsPedidosContent">
                ...
                <tbody>
                    <?php foreach ($ordenes as $o):
                        ...
                        <tr>
                            ...
                            <td><?php
                                switch ($o['estado']) {
                                  case 'pendiente':        $color='warning';  $texto='Pendiente';  break;
                                  case 'proceso_surtido':  $color='info';     $texto='En Proceso'; break;
                                  case 'proceso':          $color='info';     $texto='En Proceso'; break; /* por si acaso */
                                  case 'Listo_entrega':   $color='success'; $texto='Listo';      break;
                                  case 'entregado':       $color='primary'; $texto='Entregado';  break;
                                  default:                $color='secondary'; $texto=ucfirst(str_replace('_',' ',$o['estado']));
                                }
                                echo "<span class='badge bg-{$color}'>{$texto}</span>";
                            ?></td>
                            <td><?= htmlspecialchars($o['creador']) ?></td>
                            <td><?= $o['incluye_paquete'] ? 'Sí' : 'No' ?></td>
                            <td><span class="badge bg-<?= ($o['avance']==100?'primary':($o['avance']>0?'info':'secondary')) ?>"><?= intval($o['avance']) ?>%</span></td>
                            ...
                            <td style="white-space: nowrap;">
                                ...
                                <?php if (intval($o['avance']) < 100): ?>
                                  <a href="status_pedido.php?id=<?= $o['id'] ?>" class="btn btn-sm btn-outline-primary">Surtir</a>
                                <?php endif; ?>
                                
                                <!-- Corregir $row -> $o -->
                                <a href="editar_orden_venta.php?id=<?= (int)$o['id'] ?>" class="me-3">Editar</a>
    
                                
                                <?php if (in_array($o['estado'], ['Listo_entrega','entregado'], true)): ?>
                                  <a href="ver_entrega_pdf.php?id=<?= $o['id'] ?>" target="_blank" class="btn btn-sm btn-outline-success"><?= $o['estado'] === 'entregado' ? 'PDF' : 'Entrega PDF' ?></a>
                                <?php endif; ?>
                                <?php if ($o['estado'] === 'Listo_entrega'): ?>
                                  <a href="confirmar_entrega.php?id=<?= $o['id'] ?>" onclick="return confirm('¿Confirmar entrega?');" class="btn btn-sm btn-outline-primary">Confirmar entrega</a>
                                <?php endif; ?>
        ...
        <div class="tab-pane fade" id="historial" role="tabpanel" aria-labelledby="historial-tab">
            <table class="table table-striped align-middle">
                <thead><tr><th>#</th><th>Cliente</th><th>Creada</th><th>Entrega</th><th>Estado</th><th>Creador</th><th>Paquetería</th><th>Acciones</th></tr></thead>
                <tbody>
                    <?php foreach ($ordenes as $o):
                        if ($o['estado'] !== 'entregado') continue;
                        $porc = $o['total_pedida'] > 0 ? round($o['total_surtida'] / $o['total_pedida'] * 100) : 0; ?>
...
<script>
    function fetchPresentations(selectElement) {
        ...
        fetch(`ordenes_venta.php?action=get_presentations_by_product&producto_id=${productId}`)
            ...
            .then(presentations => {
                ...
                // datasets para validaciones y resumen
                    option.dataset.stock = Number(pres.stock || 0);
                    option.dataset.apartado = Number(pres.apartado || 0);
                    option.dataset.disponible = Number(pres.disponible || 0);
                    option.textContent = `${pres.nombre} (Disponibles: ${Number(pres.disponible).toFixed(2)} pzas)`;
                    // útil para el resumen y validaciones
    ...
    // Helper: clonar y ACTIVAR una fila desde la plantilla
        function cloneActiveRow() {
            const row = templateRow.cloneNode(true);
            row.removeAttribute('id');
            row.style.display = 'table-row';
            // Habilitar campos (la plantilla está disabled para que no valide)
            row.querySelectorAll('select, input').forEach(el => {
                el.disabled = false;
            });
            // Listeners
            row.querySelector('.remove-linea').addEventListener('click', function() {
                if (lineasBody.querySelectorAll('.remove-linea').length > 1) row.remove();
            });
            return row;
        }

        const addFirstRow = () => {
            const newRow = cloneActiveRow();
        ...
        document.getElementById('addLinea').addEventListener('click', function() {
            ...
            newRow.querySelector('.presentation-select').innerHTML = '<option value="">-- Elige presentación --</option>';
        ...
        if (!openConfirmBtn || !confirmSaveBtn || !form) {
            console.error('Faltan elementos clave del formulario de pedido.');
            return;
        }
        ...
        openConfirmBtn.addEventListener('click', function() {
            ...
            // El select de distribuidor está en el formulario de filtros (GET), fuera de #pedido-form
            const distSelect = document.querySelector('select[name="distribuidor_id"]');
            const distName = (distSelect && distSelect.value)
                ? distSelect.options[distSelect.selectedIndex].textContent.trim()
                : 'N/A';
            let summaryHtml = '<strong>Distribuidor:</strong> ' + distName + '<br>';
            ...
            form.querySelectorAll('#lineasBody tr:not([style*="display: none"])').forEach(row => {
                const prodTxt = row.querySelector('.producto-select option:checked')?.textContent || '';
                const presOpt = row.querySelector('.presentation-select option:checked');
                const presTxt = presOpt ? presOpt.textContent : '';
                const cant    = Number(row.querySelector('input[name="cantidad[]"]').value || 0);
                const disp    = presOpt ? Number(presOpt.dataset.disponible || 0) : 0;
                const falt    = Math.max(0, cant - disp);
                if (prodTxt && presTxt && cant) {
                    summaryHtml += `<li>${cant} x ${prodTxt} (${presTxt}) — Disp: ${disp} · Faltante: ${falt}</li>`;
                }
            });
            summaryHtml += '</ul>';
            document.getElementById('order-summary').innerHTML = summaryHtml;
            // Si hay faltantes, advertimos que se generará requerimiento de producción
            if ([...form.querySelectorAll('#lineasBody .presentation-select option:checked')].some(o => Number(o?.dataset?.disponible || 0) < Number(o?.closest('tr')?.querySelector('input[name="cantidad[]"]')?.value || 0))) {
                const warn = document.createElement('div');
                warn.className = 'alert alert-warning mt-2';
                warn.innerHTML = 'Hay faltantes. Se registrará <strong>requerimiento de producción</strong> por la diferencia.';
                document.getElementById('order-summary').appendChild(warn);
            }
            confirmModal.show();
        });
       ...
       confirmSaveBtn.addEventListener('click', function() {
            const formData = new FormData(form);
            // Asegurar que el backend entre a la rama AJAX
            formData.set('crear_venta_ajax', '1');
